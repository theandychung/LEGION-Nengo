...
class Inhibitor(nengo.Network):
    """A global inhibitor contains only one neuron. It inhibits the excitation of all oscillators in LEGION.
    The more oscillators are oscillating, the stronger the inhibition will be generated from the global inhibitor.

    Parameters
    ---
    All parameters are set in const.py.
    Adjustable parameters are: tau, syn, phi, theta_z
    """
    ...
    def inhibitorToLocal(self, Local):
        ...
        def threshold_z(x):
            if x >= theta_z:
                ...
                x = 0
            return x
        nengo.Connection(Local,self.input, function=threshold_z, synapse=self.syn)
